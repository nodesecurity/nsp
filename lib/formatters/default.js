...
module.exports = function (err, data, pkgPath, exceptions) {
  ...
  if (data.length === 0) {
    if (exceptions.length > 0) {
      return Chalk.yellow('(+)') + ' No known vulnerabilities found (' + exceptions.length + ' ignored)';
    }
    return Chalk.green('(+)') + ' No known vulnerabilities found';
  }

  returnString += Chalk.red('(+) ') + data.length + ' ' + (data.length === 1 ? 'vulnerability' : 'vulnerabilities') + ' found\n';
