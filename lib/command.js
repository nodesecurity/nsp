...
exports.wrap = function (name, handler) {
  ...
  return function (args) {
    ...
    let userConfig;
    try {
      userConfig = JSON.parse(StripJsonComments(Fs.readFileSync(Path.join(Os.homedir(), '.nsprc'), { encoding: 'utf8' })));
    }
    catch (err) {}
    ...
    let config;
    try {
      config = JSON.parse(StripJsonComments(Fs.readFileSync(Path.join(args.path, '.nsprc'), { encoding: 'utf8' })));
    }
    catch (err) {}
